...
class FlashSACDoubleCritic(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.num_qs = num_qs
        # SAPG: the critic is deliberately block-UNconditioned. SAC's replay buffer already
        # aggregates every block's data, and Bellman targets are computed under the leader
        # policy, so a single shared Q (the leader's soft Q) is trained on all blocks'
        # transitions — the SAC analogue of SAPG's leader-follower aggregation.
    ...
    def forward(
        ...
        x = torch.cat((observations, actions), dim=-1).contiguous()  # [B, obs+act]
