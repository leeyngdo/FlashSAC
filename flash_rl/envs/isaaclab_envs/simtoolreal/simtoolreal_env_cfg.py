...
Vendored from tylerlum/simtoolreal; the only FlashSAC change is anchoring the asset
URDF paths on the package ``ASSET_DIR`` (upstream resolves them against the cwd).

Organized into sectioned sub-configclasses:
...
Values match the upstream isaacgymenvs/cfg/task/SimToolReal.yaml defaults with
the following deliberate upstream deviations:
...
class AssetsCfg:
    robot_urdf: str = (
        f"{ASSET_DIR}/simtoolreal_description/urdf/kuka_sharpa_description/"
        "iiwa14_left_sharpa_adjusted_restricted.urdf"
    )
    table_urdf: str = f"{ASSET_DIR}/simtoolreal_description/urdf/table_narrow.urdf"
