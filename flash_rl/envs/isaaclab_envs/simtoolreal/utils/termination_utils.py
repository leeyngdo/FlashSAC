...
def update_tolerance_curriculum(env) -> None:
    ...
    # FlashSAC runs eval on the training env instance; freeze the one-way curriculum
    # during eval so deterministic rollouts cannot tighten it.
    if getattr(env, "eval_mode", False):
