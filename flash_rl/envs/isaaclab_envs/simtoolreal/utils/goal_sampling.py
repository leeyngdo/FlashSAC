...
def _scale_workspace_bounds(mins: torch.Tensor, maxs: torch.Tensor, scale: float) -> tuple[torch.Tensor, torch.Tensor]:
...
def sample_delta_goal_pose(
    ...
    angle = (torch.rand(n, device=device) * 2.0 - 1.0) * delta_rotation_degrees * (math.pi / 180.0)
