...
def log_step_metrics(env) -> None:
    ...
    episode_final.update({f"done_{name}": value.float() for name, value in env._termination_reasons.items()})
    ...
    # FlashSAC's IsaacLab wrapper only surfaces scalar metrics under extras["log"];
    # mirror the key metrics there.
    env.extras["log"] = {
        "episode/prev_successes_mean": env._prev_episode_successes.float().mean(),
        "curriculum/success_tolerance": float(env._current_success_tolerance),
        **{f"done_rate/{name}": value.float().mean() for name, value in env._termination_reasons.items()},
        **{f"reward_step_mean/{name}": value.mean() for name, value in env._reward_terms.items()},
    }
