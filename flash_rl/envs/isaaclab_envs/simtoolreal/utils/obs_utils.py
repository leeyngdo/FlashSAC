...
def _perturb_quat(q_wxyz: torch.Tensor, max_deg: float) -> torch.Tensor:
    ...
    n = q_wxyz.shape[0]
    axis = torch.nn.functional.normalize(torch.randn(n, 3, device=q_wxyz.device), dim=-1)
    angle = torch.empty(n, device=q_wxyz.device).uniform_(-max_deg, max_deg) * (math.pi / 180.0)
...
def _apply_local_offset(
    ...
    shifted = quat_apply(rot_wxyz.reshape(-1, 4), offset_t.reshape(-1, 3)).reshape(*batch_shape, 3)
...
def _canonical_joint_obs(env) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    joint_pos = 2.0 * (joint_pos_raw - env._joint_lower_canon) / (env._joint_upper_canon - env._joint_lower_canon) - 1.0
...
def compute_intermediate_values(env) -> None:
    ...
    env._curr_fingertip_distances = torch.norm(ft_pos - obj_pos.unsqueeze(1), dim=-1)  # (N, 5)
    ...
    env._closest_keypoint_max_dist = torch.where(sentinel, env._keypoints_max_dist, env._closest_keypoint_max_dist)
    sentinel_ft = env._closest_fingertip_dist < 0.0
    env._closest_fingertip_dist = torch.where(sentinel_ft, env._curr_fingertip_distances, env._closest_fingertip_dist)
...
def _apply_object_state_dr(env, obj_pos, obj_rot, obj_linvel, obj_angvel):
    ...
    env._object_state_queue, delayed = _sample_delay(env._object_state_queue, state, env, flush=_episode_start(env))
...
def _apply_obs_delay(env, policy_tensor: torch.Tensor) -> torch.Tensor:
    ...
    env._obs_queue, delayed = _sample_delay(env._obs_queue, policy_tensor, env, flush=_episode_start(env))
    return delayed
...
def build_observations(env) -> dict[str, torch.Tensor]:
    ...
    palm_center_pos_w = _apply_local_offset(palm_pos_w, palm_rot, PALM_CENTER_OFFSET, (env.num_envs,))
    ...
    if goal_yaw_obs_noise is not None and torch.any(goal_yaw_obs_noise != 0):
        z_axis = (
            torch.tensor([0.0, 0.0, 1.0], device=goal_rot.device, dtype=goal_rot.dtype)
            .unsqueeze(0)
    ...
    fingertip_pos_rel_palm = (ft_pos_w - env_origins.unsqueeze(1)) - palm_pos.unsqueeze(1)  # (N, 5, 3)
    ...
    if dr.joint_velocity_obs_noise_std > 0:
        obs_noisy["joint_vel"] = joint_vel + torch.randn_like(joint_vel) * dr.joint_velocity_obs_noise_std
...
def build_student_observations(env) -> dict[str, torch.Tensor]:
    ...
    cfg = env.cfg.student_obs
    if not cfg.enabled:
        raise RuntimeError("cfg.student_obs.enabled is false; student observations are disabled.")
