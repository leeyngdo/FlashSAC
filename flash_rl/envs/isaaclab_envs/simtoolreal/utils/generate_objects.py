...
def _cylinder_urdf_constant_density(path: Path, height: float, diameter: float, density: float) -> Path:
    radius = diameter / 2
...
def generate_handle_urdf(path: Path, handle_scale: Scale, handle_density: float = 400.0) -> Path:
    ...
    if len(handle_scale) == 2:
        return _cylinder_urdf_constant_density(path, handle_scale[0], handle_scale[1], handle_density)
...
def _handle_head_urdf_variable_density(
    ...
    if len(handle_scale) == 3:
        ...
        handle_mass, handle_ixx, handle_iyy, handle_izz = _compute_mass_and_inertia(handle_scale, handle_density)
    else:
        h, d = handle_scale
        r = d / 2
        handle_geom = (
        ...
        handle_mass, handle_izz, handle_iyy, handle_ixx = _compute_mass_and_inertia(handle_scale, handle_density)
    ...
    if len(head_scale) == 3:
        ...
        head_mass, head_ixx, head_iyy, head_izz = _compute_mass_and_inertia(head_scale, head_density)
    else:
        hh, hd = head_scale
        hr = hd / 2
        ...
        head_mass, head_ixx, head_izz, head_iyy = _compute_mass_and_inertia(head_scale, head_density)
...
def generate_handle_head_urdf(
    ...
    if head_scale is not None and head_density is not None:
        return _handle_head_urdf_variable_density(path, handle_scale, head_scale, handle_density, head_density)
    raise ValueError(
        f"head_scale and head_density must both be set or both None (got " f"{head_scale} and {head_density})"
    )
...
def generate_handle_head_urdfs(
    ...
    for dist in matching:
        ...
        for idx in range(num_per_type):
            ...
            fname = (
                f"{idx:03d}_{dist.type}_handle_{h_scale}_head_{head}_d{h_d:.1f}_{head_d}".replace(".", "-") + ".urdf"
            )
    ...
    scales_norm = [(x / object_base_size, y / object_base_size, z / object_base_size) for (x, y, z) in scales_3d]
