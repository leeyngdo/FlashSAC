...
def sample_log_uniform(lo_hi: tuple[float, float], n: int) -> torch.Tensor:
    ...
    lo, hi = lo_hi
    return torch.exp(torch.empty(n).uniform_(math.log(lo + 1e-12), math.log(hi + 1e-12)))
...
def apply_action_pipeline(env, actions: torch.Tensor) -> None:
    ...
    if dr.use_action_delay and dr.action_delay_max > 0:
        ...
        delay_idx = torch.randint(0, env._action_queue.shape[1], (env.num_envs,), device=env.device)
        actions = env._action_queue[torch.arange(env.num_envs, device=env.device), delay_idx]
    ...
    arm_smoothed = act_cfg.arm_moving_average * arm_raw + (1.0 - act_cfg.arm_moving_average) * env._prev_targets[:, :7]
    arm_smoothed = torch.clamp(arm_smoothed, env._arm_lower, env._arm_upper)

    # Hand: absolute [-1, 1] scale.
    hand_action = actions[:, 7:]
    hand_raw = env._hand_lower + 0.5 * (hand_action + 1.0) * (env._hand_upper - env._hand_lower)
    hand_smoothed = (
        act_cfg.hand_moving_average * hand_raw + (1.0 - act_cfg.hand_moving_average) * env._prev_targets[:, 7:]
    )
...
def apply_wrench_dr(env) -> None:
    ...
    force_fire = (torch.rand(env.num_envs, device=env.device) < env._random_force_prob).view(-1, 1, 1)
    torque_fire = (torch.rand(env.num_envs, device=env.device) < env._random_torque_prob).view(-1, 1, 1)
    mass = env._object_mass.unsqueeze(-1)  # (N, 1, 1)
    new_force = torch.randn(env.num_envs, 1, 3, device=env.device) * mass * dr.force_scale
    new_torque = torch.randn(env.num_envs, 1, 3, device=env.device) * mass * dr.torque_scale
