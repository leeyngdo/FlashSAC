...
def build_robot_articulation_usd_cfg(usd_path: str, *, start_arm_higher: bool = False) -> ArticulationCfg:
...
def _student_camera_data_types(modality: str) -> list[str]:
    ...
    raise ValueError("cfg.student_obs.image_modality must be one of " f"('depth', 'rgb', 'rgbd'), got {modality!r}.")
...
def _diagnose_target_expr(expr: str) -> None:
    ...
    """
    from isaaclab.sim.utils import find_first_matching_prim, find_matching_prims
    ...
    if backend in ("tiled", "standard"):
        ...
        camera_cfg = camera_cfg_cls(
            ...
            spawn=sim_utils.PinholeCameraCfg(
                ...
                horizontal_aperture_offset=float(getattr(cfg, "horizontal_aperture_offset", 0.0)),
                vertical_aperture_offset=float(getattr(cfg, "vertical_aperture_offset", 0.0)),
        ...
        for expr in tuple(cfg.raycast_static_prim_exprs):
            ...
            raycast_targets.append(
                MultiMeshRayCasterCameraCfg.RaycastTargetCfg(prim_expr=str(expr), track_mesh_transforms=False)
            )
        for expr in tuple(cfg.raycast_dynamic_prim_exprs):
            expanded = _expand_link_wildcard(str(expr), env.num_envs)
            print(
                f"[raycaster] DYNAMIC target: {expr!r} -> " f"{len(expanded)} concrete: {expanded}",
                flush=True,
            )
            for concrete_expr in expanded:
                ...
                raycast_targets.append(
                    MultiMeshRayCasterCameraCfg.RaycastTargetCfg(prim_expr=concrete_expr, track_mesh_transforms=True)
                )
        ...
        for env_id in range(int(env.num_envs)):
            sim_utils.create_prim(f"/World/envs/env_{env_id}/StudentCamera", "Xform")
        ...
        right_pos, right_quat = _stereo_right_pose_from_left(left_pos, left_quat, float(cfg.fs_stereo_baseline_m))
        ...
        env.student_camera_left = TiledCamera(
            cfg=_build_stereo_cam_cfg(
                "/World/envs/env_.*/StudentCameraLeft",
                left_pos,
                left_quat,
            )
        )
        env.student_camera_right = TiledCamera(
            cfg=_build_stereo_cam_cfg(
                "/World/envs/env_.*/StudentCameraRight",
                right_pos,
                right_quat,
            )
        )
        ...
        print(
            ...
            f"[foundation_stereo]              right pos={right_pos} "
            f"(baseline {cfg.fs_stereo_baseline_m:.3f} m along left +X)\n"
        ...
        size_str = f"stereo {stereo_w}x{stereo_h} -> " f"{int(cfg.image_width)}x{int(cfg.image_height)} via Fast-FS"

    _log_scene_step(
        t0,
        f"registered student camera backend={backend} " f"modality={cfg.image_modality} " f"size={size_str}",
    )
...
def _draw_depth_sticks(
    ...
    *,
    ...
    w_half_max = max_w // 2
    2 * w_half_max + 1  # kernel side, matches the old `out[..., y±w_half, x±w_half]` slice
    lo = float(cfg.depth_aug_randu_min_m)
    hi = float(cfg.depth_aug_randu_max_m)
    ...
    within_w = (dx_g[None, None, None].abs() <= w_half_b) & (
        dy_g[None, None, None].abs() <= w_half_b
    )  # (B, S, 1, K, K)
...
def _preprocess_student_depth(env, depth: torch.Tensor) -> torch.Tensor:
    ...
    if far <= near:
        raise ValueError("cfg.student_obs.depth_max_m must be greater than " "cfg.student_obs.depth_min_m.")
...
def _run_foundation_stereo(env, cfg) -> torch.Tensor:
    ...
    if left_rgba is None or right_rgba is None:
        raise RuntimeError("foundation_stereo backend: one of student_camera_{left,right} " "produced no RGB output.")
    ...
    fx_px = float(cfg.fs_stereo_width) * float(cfg.focal_length) / float(cfg.horizontal_aperture)
...
def read_student_camera_image(env) -> torch.Tensor:
    ...
    if cfg is None or not cfg.enabled:
        raise RuntimeError("cfg.student_obs.enabled is false; no student image is available.")
    if not cfg.image_enabled:
        raise RuntimeError("cfg.student_obs.image_enabled is false; no student image is available.")
    ...
    if str(cfg.camera_backend).lower() == "foundation_stereo":
        if str(cfg.image_modality).lower() != "depth":
            raise ValueError(
                f"camera_backend='foundation_stereo' requires " f"image_modality='depth' (got {cfg.image_modality!r})."
            )
        env.sim.render()
        ...
        depth_policy = _crop_student_image(env, _preprocess_student_depth(env, depth))
    ...
    if camera is None:
        raise RuntimeError(
            "Student camera was not created. Check cfg.student_obs.enabled and " "launch with cameras enabled."
        )
    ...
    if modality in ("rgb", "rgbd"):
        ...
        if rgb is None:
            raise RuntimeError(f"RGB output missing. Available student camera outputs: {list(available.keys())}")
    ...
    if modality in ("depth", "rgbd"):
        ...
        if depth is None:
            raise RuntimeError(f"Depth output missing. Available student camera outputs: {list(available.keys())}")
...
def _load_adjacent_links_map() -> dict[str, list[str]]:
    """Load the link adjacency map (the link pairs whose self-collision must be
    filtered) from the vendored :mod:`..adjacent_links` and merge LEFT+RIGHT.

    Merging both handednesses is safe: links absent from the imported robot
    simply find no prim and are skipped.
    """
    from ..adjacent_links import (
...
def _generate_scaled_table_urdfs(
    ...
    if len(base_dims) != 3:
        raise ValueError(f"expected 3-element <box size>, got {base_dims!r} from {base_urdf_path}")
    ...
    for prim in Usd.PrimRange(root):
        ...
        if prim.HasAPI(UsdPhysics.CollisionAPI):
            ...
            if collision_enabled is not None:
                ...
                (ce.GetCollisionEnabledAttr() or ce.CreateCollisionEnabledAttr()).Set(collision_enabled)
...
def setup_scene(env) -> None:
    ...
    _log_scene_step(
        setup_t0,
        f"setup start num_envs={env.num_envs} " f"num_assets_per_type={assets_cfg.num_assets_per_type}",
    )
    ...
    if not urdf_paths:
        raise ValueError(
            "No procedural object URDFs were generated. " "Check cfg.assets.handle_head_types and num_assets_per_type."
        )
    ...
    table_scale_is_trivial = (scale_range_x == (1.0, 1.0) and scale_range_y == (1.0, 1.0)) or n_table_variants <= 1
