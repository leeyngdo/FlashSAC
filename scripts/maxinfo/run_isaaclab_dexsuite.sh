#!/bin/bash -ex
# MaxInfoSAC on Kuka-Allegro Reorient, matching the IsaacLab benchmark defaults
# (scripts/run_isaaclab.sh). Usage: run_isaaclab_dexsuite.sh [gpu] [seed]
...
CUDA_VISIBLE_DEVICES=${1:-0} uv run --no-sync python train.py \
  ...
  --overrides num_train_envs=1024 --overrides num_eval_envs=null --overrides num_record_envs=null \
